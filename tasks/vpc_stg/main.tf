...
variable "subnet_private_common_cidr_blocks" { type = "list" }
variable "subnet_private_web_cidr_blocks" { type = "list" }
variable "subnet_private_db_cidr_blocks" { type = "list" }
variable "subnet_private_cache_cidr_blocks" { type = "list" }
...
# Subnet private web
module "subnet_private_web" {
  source = "../../modules/subnet"

  aws_subnet_variables {
    name   = "${var.env == "prd" ? "${var.project_name}-subnet-private-web%02d" : "${var.project_name}-${var.env}-subnet-private-web%02d"}"
    vpc_id = "${module.vpc.vpc_id}"
    type   = "web"
  }

  cidr_blocks        = "${var.subnet_private_web_cidr_blocks}"
  availability_zones = "${var.subnet_availability_zones}"
}
...
module "subnet_private_db" {
  ...
  aws_subnet_variables {
    name   = "${var.env == "prd" ? "${var.project_name}-subnet-private-db%02d" : "${var.project_name}-${var.env}-subnet-private-db%02d"}"
    vpc_id = "${module.vpc.vpc_id}"
    type   = "db"
  }

  cidr_blocks        = "${var.subnet_private_db_cidr_blocks}"
  availability_zones = "${var.subnet_availability_zones}"
}

# Subnet private cache
module "subnet_private_cache" {
  source = "../../modules/subnet"

  aws_subnet_variables {
    name   = "${var.env == "prd" ? "${var.project_name}-subnet-private-cache%02d" : "${var.project_name}-${var.env}-subnet-private-cache%02d"}"
    vpc_id = "${module.vpc.vpc_id}"
    type   = "cache"
  }

  cidr_blocks        = "${var.subnet_private_cache_cidr_blocks}"
  availability_zones = "${var.subnet_availability_zones}"
}
...
module "route_table_association_private" {
  ...
  subnet_ids = ["${concat(split(",", module.subnet_private_common.subnet_ids), split(",", module.subnet_private_web.subnet_ids), split(",", module.subnet_private_db.subnet_ids), split(",", module.subnet_private_cache.subnet_ids))}"]
}
